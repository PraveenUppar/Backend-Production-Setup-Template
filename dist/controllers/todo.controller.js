...
exports.patchTodoController =
  exports.deleteTodoController =
  exports.getTodoController =
  exports.postTodoController =
    void 0;
...
const postTodoController = async (req, res) => {
  ...
  try {
    ...
    const newTodo = await (0, todo_service_1.createTodoService)(userId, {
      title,
      completed,
    });
...
const patchTodoController = async (req, res) => {
  try {
    ...
    const { title, completed } = req.body;
    const updateTodo = await (0, todo_service_1.patchTodoService)(id, {
      title,
      completed,
    });
    res.status(200).json({
